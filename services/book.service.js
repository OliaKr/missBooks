...
import { asystorageService } from './async-storage.service.js'
...
function query(filterBy = getDefaultFilter()) {
  ...
  return asystorageService.query(BOOK_KEY)
    .then(books => {

      if (!books || !books.length) {
        books = gBooks
        asystorageService.save(BOOK_KEY, books)
      }
      if (filterBy.txt) {
...
function get(bookId) {
  return asystorageService.get(BOOK_KEY, bookId)
...
function remove(bookId) {
  return asystorageService.remove(BOOK_KEY, bookId)
}

function save(book) {
  if (book.id) {
    return asystorageService.put(BOOK_KEY, book)
  } else {
    return asystorageService.post(BOOK_KEY, book)
  }
}
